...
class AuthControllerTest {
        ...
        @Test
        @DisplayName("Deve retornar 200 quando a senha for recuperada com sucesso")
        void deveRetornar200QuandoRecuperarSenhaComSucesso() throws Exception {
                String resetToken = "valid-reset-token";
                String newPassword = "NovaSenha123!";

                when(authService.resetPassword(resetToken, newPassword)).thenReturn(true);
                ...
                mockMvc.perform(MockMvcRequestBuilders
                                ...
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.success").value(true))
                                .andExpect(jsonPath("$.message").value("Senha recuperada com sucesso"))
                                .andExpect(jsonPath("$.data.resetToken").value(resetToken));
        }

        @Test
        @DisplayName("Deve retornar 200 mesmo se a recuperação de senha falhar")
        void deveRetornar200QuandoFalhaRecuperacaoSenha() throws Exception {
                String resetToken = "invalid-reset-token";
                String newPassword = "Senha123!";

                when(authService.resetPassword(resetToken, newPassword)).thenReturn(false);
