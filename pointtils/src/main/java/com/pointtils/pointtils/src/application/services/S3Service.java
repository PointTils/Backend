...
public class S3Service {
    ...
    public boolean deleteFile(String fileUrl) {
        if (!s3Enabled) {
            throw new UnsupportedOperationException(
                "Deleção de arquivos para S3 está desabilitado. Configure spring.cloud.aws.s3.enabled=true para habilitar."
            );
        }
    ...
    private String extractKeyFromUrl(String url) {
        if (url.contains(".s3.")) {
            // Exemplo: https://test-bucket.s3.us-east-1.amazonaws.com/users/123/test.jpg
            return url.substring(url.indexOf(".com/") + 5);
        } else if (url.contains("s3.amazonaws.com/")) {
            // Exemplo: https://s3.amazonaws.com/test-bucket/users/123/test.jpg
