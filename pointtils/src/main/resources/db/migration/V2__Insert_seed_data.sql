...
SELECT id, 'SP', 'São Paulo' FROM users WHERE email = 'client1@email.com';
...
INSERT INTO location (user_id, UF, city)
SELECT id, 'RJ', 'Rio de Janeiro' FROM users WHERE email = 'empresa1@email.com';
...
INSERT INTO location (user_id, UF, city)
SELECT id, 'MG', 'Belo Horizonte' FROM users WHERE email = 'interpreter1@email.com';
...
-- ========================
WITH client AS (
  SELECT id AS client_id FROM users WHERE email = 'client1@email.com'
),
interp AS (
  SELECT id AS interpreter_id FROM users WHERE email = 'interpreter1@email.com'
)
